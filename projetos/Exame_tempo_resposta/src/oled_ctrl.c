...
void oled_times_print(int8_t nivel,int8_t contador_turnos,float *tempo_turnos, bool *sucess,uint8_t start_addry){
    ...
    for(uint8_t i=0; i< contador_turnos ;i++){//for para escrever todos os tempos pegos
        ...
        sprintf(buffer,"T%d %.3f %c",i,tempo_turnos[i],turn_flag);
