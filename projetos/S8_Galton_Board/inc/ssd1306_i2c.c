...
void ssd1306_draw_rect(uint8_t *ssd, uint8_t x_start, uint8_t y_start, uint8_t x_end, uint8_t y_end,
    bool mode) {
    ssd1306_draw_line(ssd, x_start, y_start, x_end, y_start, mode);
    ssd1306_draw_line(ssd, x_start, y_end, x_end, y_end, mode);
    ssd1306_draw_line(ssd, x_start, y_start, x_start, y_end, mode);
    ssd1306_draw_line(ssd, x_end, y_start, x_end, y_end, mode);
}

void ssd1306_fill_rect(uint8_t *ssd, uint8_t x_start, uint8_t y_start, uint8_t x_end, uint8_t y_end,bool mode) {
    for (uint8_t x = x_start; x <= x_end; x++) {
        for (uint8_t y = y_start; y <= y_end; y++) {
            ssd1306_set_pixel(ssd, x, y, mode);
        }
    }
}
